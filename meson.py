...
for page in range(total_pages):
    ...
    if response.status_code == 200:
        ...
        transactions = data['result']['list'] if 'result' in data and 'list' in data['result'] else []
        ...
        for transaction in transactions:
            ...
            event_name = transaction["events"][0]["name"] if "events" in transaction and len(transaction["events"]) > 0 else None
            initiator = transaction["initiator"] if "initiator" in transaction else None
            from_address = transaction["fromTo"][0] if "fromTo" in transaction and len(transaction["fromTo"]) > 0 else None
            to_address = transaction["fromTo"][1] if "fromTo" in transaction and len(transaction["fromTo"]) > 1 else None
...
all_records_df['created'] = pd.to_datetime(all_records_df['created']).dt.strftime('%Y-%m-%d %H:%M:%S')
