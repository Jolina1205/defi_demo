...
def get_symbiosis_data(skip=0):
    ...
    for record in actual:
        ...
        keys_to_extract = ['id', 'from_client_id', 'from_chain_id', 'join_chain_id',
                           'to_chain_id', 'event_type', 'type', 'state', 'created_at', 'mined_at',
                           'success_at',  'duration', 'amounts',
                           'from_amount_usd','to_amount_usd','to_tx_id', 'retry_active']
        ...
        symbiosis_data = pd.concat([symbiosis_data, pd.DataFrame([top_level_fields])], ignore_index=True)

    # Convert the 'created_at', 'mined_at', and 'success_at' columns to a string format that Excel can recognize as a date and time
    symbiosis_data['created_at'] = symbiosis_data['created_at'].dt.strftime('%Y-%m-%d %H:%M:%S')
    symbiosis_data['mined_at'] = symbiosis_data['mined_at'].dt.strftime('%Y-%m-%d %H:%M:%S')
    symbiosis_data['success_at'] = symbiosis_data['success_at'].dt.strftime('%Y-%m-%d %H:%M:%S')
...
while True:
    ...
    if len(all_records_df) >= total_transactions_march_2024:
        print(f"Reached the total number of transactions for March 2024: {total_transactions_march_2024}")
        break
